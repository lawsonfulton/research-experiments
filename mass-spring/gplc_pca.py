...
def main():
    ...
    def DEL(new_q, cur_q, prev_q):
        ...
        res = D1_Ld(cur_q, new_q) + D2_Ld(prev_q, cur_q) + mass_matrix @ gravity_accel
    ...
    def grad(new_z, cur_z, prev_z):
        ...
        return J.T @ mass_matrix @ A - h * J.T @ internal_forces(new_q) - h * J.T @ F_ext

    print(grad(z_initial, z_initial, z_initial))
    print(autograd.jacobian(gplc_objective,0)(z_initial, z_initial, z_initial))
    ...
    while True:
        ...
        sol = optimize.minimize(gplc_objective, cur_z, args=(cur_z, prev_z), method='L-BFGS-B', jac=grad)#, options={'disp':True})
        prev_z = cur_z
        cur_z = sol.x
